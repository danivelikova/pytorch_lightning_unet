...
class DiceLoss(nn.Module):
    ...
    """
        ...
        if num_classes == 1:
            ...
            # pos_prob = torch.ge(pos_prob, 0.5).float() #binary output ->> gradient detached
            # withouth thresholding is soft DICE loss
            pos_prob.data = torch.ge(pos_prob.data, 0.5).float() #binary output -> DICE loss
            neg_prob = 1 - pos_prob
